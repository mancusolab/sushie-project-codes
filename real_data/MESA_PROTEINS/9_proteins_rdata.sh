...
params=`sed "${NR}q;d"  ${SCRATCH}/mesa_proteins_gene_list_noMHC.tsv`
set -- junk $params
shift
ID=${15}
CHR=$4
...
echo SUMMARY ${NR}, ${ID}, ${CHR}, ${P0}, ${P1}

TMPDIR=/scratch1/zeyunlu/temp_rdata_proteins/${ID}
...
for pop in EUR AFR HIS
  ...
  echo "Getting geno and pheno data for ${pop}"
  KEEP=${SCRATCH}/mesa_proteins_pt_${pop}.tsv
  Rscript ${SCRIPTF}/sushie-data-codes/real_data/utils/split_people.R $KEEP $((NR + NR*60000)) $TMPDIR/${ID}.${pop}.split
  PHENO=${SCRATCH}/mesa_proteins_v1_${pop}.tsv.gz
