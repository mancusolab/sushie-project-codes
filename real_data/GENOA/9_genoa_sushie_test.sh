...
do
 ...
 --output $TMPDIR/${ID}.normal
  ...
  ${PLINK} --bfile $TMPDIR/${ID}_ea_plink --extract  $TMPDIR/${ID}.snp --keep ${ea_pt} --glm 'hide-covar' omit-ref \
    --covar  $ea_covar2 --covar-variance-standardize --pheno $TMPDIR/${ID}_ea_pheno.sushie.bed2 \
    --out ${TMPDIR}/${ID}.EUR

  ${PLINK} --bfile $TMPDIR/${ID}_aa_plink --extract  $TMPDIR/${ID}.snp --keep ${aa_pt} --glm 'hide-covar' omit-ref \
    --covar $aa_covar2 --covar-variance-standardize --pheno $TMPDIR/${ID}_aa_pheno.sushie.bed2 \
    --out ${TMPDIR}/${ID}.AFR

  # run mesusie
  python ${SCRIPTF}/sushie-data-codes/real_data/utils/run_multisusie_2pop_test.py \
  ...
  ss2=$(awk 'NR==2 {print $1}' $TMPDIR/${ID}.tmp.md.tsv)
   ...
   sushie finemap --summary \
    --gwas $TMPDIR/${ID}.tmp.inss.ans0.tsv $TMPDIR/${ID}.tmp.inss.ans1.tsv \
    --ld $TMPDIR/${ID}.tmp.inld.ans0.tsv $TMPDIR/${ID}.tmp.inld.ans1.tsv \
    --sample-size $ss1 $ss2 --alphas \
