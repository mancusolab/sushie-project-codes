...
do
  ...
  if find "/scratch1/zeyunlu/sushie_genoa/sushie/alphas/" -maxdepth 1 -name "${ID}.*.alphas.tsv" -print -quit | grep -q .;
  then
    python /project/nmancuso_8/zeyunlu/projects/sushie-data-codes/real_data/utils/enrich_prepare.py \
      --weights-path /scratch1/zeyunlu/sushie_genoa/ \
      --ans 2 \
      --gene $ID \
    ...
    cp ${SCRATCH}/enrich_header.tsv /scratch1/zeyunlu/sushie_genoa/enrich/enrich_genoa_${ID}.tsv
    find $TMPDIR -name "enrich.${ID}.*.final.tsv" | xargs -n 1 tail -n +2 >> /scratch1/zeyunlu/sushie_genoa/enrich/enrich_genoa_${ID}.tsv
  else
    echo "No CS files."
  fi
  rm -rf $TMPDIR
