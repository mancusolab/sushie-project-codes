...
for idx in range(n_pop):
    ...
    new_z2.to_csv(f"{args.tmp_out}.group{args.group}.inss.ans{idx}.tsv", index=False, sep="\t")
    tmp_ld = output_dic["trueLD"][idx][new_z1.index.values, :][:, new_z1.index.values]
    new_ld_list.append(tmp_ld)
    pd.DataFrame(tmp_ld). \
        to_csv(f"{args.tmp_out}.group{args.group}.inld.ans{idx}.tsv", index=False, sep="\t", header=None, float_format="%.12f")

# compute ldscore for xmap and mesusie
ldsc1 = np.sum(new_ld_list[0] ** 2, axis=0)
ldsc2 = np.sum(new_ld_list[1] ** 2, axis=0)
ldsc12 = np.sum(new_ld_list[0] * output_dic["trueLD"][1], axis=0)
pd.DataFrame(jnp.concatenate([ldsc1[:, jnp.newaxis], ldsc2[:, jnp.newaxis],
                             ldsc12[:, jnp.newaxis]], axis=1)). \
    to_csv(f"{args.tmp_out}.group{args.group}.inldsc.tsv", index=False, sep="\t", header=None)

df_met = pd.DataFrame({"N": sample_size,
                    "BP": [jnp.min(df_z.pos.values), jnp.max(df_z.pos.values)]})

df_met.to_csv(f"{args.tmp_out}.group{args.group}.md.tsv", index=False, sep="\t", header=None)

# multisusie1_converged = True
# try:
#     multisusie1 = MultiSuSiE.multisusie_rss(z_list=[np.array(ss_list[0].T_STAT.values), np.array(ss_list[1].T_STAT.values)],
#                                             R_list=new_ld_list[0:2], rho=np.array([[1, 0.1], [0.1, 1]]),
...
#     df_weights.to_csv(f"{args.tmp_out}.multisusie.group{args.group}.weights.tsv", index=False, sep="\t")
