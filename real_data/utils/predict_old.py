...
for idx in range(n_pop):
    ...
    _, _, _, trueLD = _compute_ld(bed[idx])
    trueLD_list.append(trueLD)
...
if n_pop == 2:
    sample_size = [geno[0].shape[0], geno[1].shape[0]]
else:
    sample_size = [geno[0].shape[0], geno[1].shape[0], geno[2].shape[0]]
...
for cv in range(cv_num):
    ...
    for idx in range(n_pop):
        tmp_z = regress(train_geno[idx], train_pheno[idx])
        tmp_z.z = tmp_z.beta / tmp_z.se
        tmp_snps = snps.copy()
        tmp_snps = tmp_snps.rename(columns={"snp": "SNP", "pos": "POS"})
        tmp_snps["Beta"] = tmp_z.beta
        tmp_snps["Se"] = tmp_z.se
        tmp_snps["Z"] = tmp_z.z
        tmp_snps["N"] = snps.shape[0]
        tmp_snps = tmp_snps[["SNP", "POS", "Beta", "Se", "Z", "N"]]
        tmp_snps.to_csv(f"{args.out}/group{cv}.ans{idx}.ss.tsv", sep="\t",index=False)
        tmp_ld = (train_geno[idx].T @ train_geno[idx])/train_geno[idx].shape[0]
        pd.DataFrame(tmp_ld).to_csv(f"{args.out}/group{cv}.ans{idx}.ld.tsv", sep="\t",index=False, header=False)
        pd.DataFrame(valid_geno[idx]).to_csv(f"{args.out}/group{cv}.ans{idx}.validx.tsv", sep="\t",index=False, header=False)
        pd.DataFrame(valid_pheno[idx]).to_csv(f"{args.out}/group{cv}.ans{idx}.validy.tsv", sep="\t",index=False, header=False)
...
df_res.columns = ["sushie", "indep", "meta", "susie", "enet", "lasso", "gblup", "cross"]
df_res["type"] = "r2"
