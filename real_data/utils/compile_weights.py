...
df_tmp_indep = pd.read_csv(f"{args.w_files}/{args.trait}.group{args.group}.indep.sushie.weights.tsv", sep="\t")
if n_pop == 2:
    df_tmp_indep["ancestry3_sushie_weight"] = 0
df_tmp_indep = df_tmp_indep[["snp", "a1","ancestry1_sushie_weight", "ancestry2_sushie_weight", "ancestry3_sushie_weight"]]
df_tmp_indep.columns = ["snp", "a1", "indep_pop1", "indep_pop2", "indep_pop3"]

df_tmp_meta = pd.read_csv(f"{args.w_files}/{args.trait}.group{args.group}.normal.meta.weights.tsv", sep="\t")
if n_pop == 2:
    df_tmp_meta["ancestry3_single_weight"] = 0
df_tmp_meta = df_tmp_meta[["snp", "a1", "ancestry1_single_weight", "ancestry2_single_weight", "ancestry3_single_weight"]]
df_tmp_meta.columns = ["snp", "a1", "meta_pop1", "meta_pop2", "meta_pop3"]
    
mesusie_paths = f"{args.w_files}/mesusie.{args.trait}.group{args.group}.tmp.weights.tsv"
# multisusie_paths = f"{args.w_files}/{args.trait}.tmp.multisusie.group{args.group}.weights.tsv"
...
df_res = df_tmp.merge(df_tmp_indep, how="inner", on=["snp","a1"]).merge(df_tmp_meta, how="inner", on=["snp","a1"]).merge(df_tmp_mega, how="inner", on=["snp", "a1"]).merge(df_tmp_mesusie, how="inner", on="snp").merge(snps, how="inner", on=["snp", "a1"])
...
for idx in range(n_pop):
    ...
    tmp_res = df_res[["snp", "a1", f"sushie_pop{idx+1}", f"indep_pop{idx+1}", f"meta_pop{idx+1}", "mega_weight", f"mesusie_pop{idx+1}", "enet_weight", "lasso_weight", "gblup_weight"]]
    df_wk = tmp_bim[["snp", "a0", "a1"]].merge(tmp_res, how="inner", on=["snp"])
    comp = ((df_wk.a1_x == df_wk.a1_y) * 1).replace(0, -1)
    all_weights = jnp.einsum("ij,i->ij", jnp.array(df_wk.iloc[:,4:12]), jnp.array(comp.astype(int)))
    imp_pheno = jnp.einsum("ij,jk->ik", tmp_bed, all_weights)
    tmp_res2 = pd.concat([tmp_fam[["iid"]], pd.DataFrame(imp_pheno)], axis=1)
    tmp_res2.columns = ["IID", "sushie", "indep", "meta", "mega", "mesusie", "enet", "lasso", "ridge"]
    tmp_res2.to_csv(f"{args.out}.{ans_list[idx]}.value.sscore", sep="\t", index=False)

# cross ancestry
cross_list = ["HIS", "EUR", "AFR"] if n_pop == 3 else ["AFR", "EUR"]
for idx in range(n_pop):
    tmp_bim, tmp_fam, tmp_bed = read_plink(f"{args.w_files}/{args.trait}.{cross_list[idx]}.test.group{args.group}.geno", verbose=False)
    tmp_bim = tmp_bim[["chrom", "snp", "a0", "a1", "i"]]
    tmp_bim = tmp_bim.merge(df_res[["snp"]], how="right", on="snp")
    tmp_bed = tmp_bed.compute()[tmp_bim.i.values, :].T
    tmp_res = df_res[["snp", "a1", f"sushie_pop{idx+1}", f"indep_pop{idx+1}", f"meta_pop{idx+1}", "mega_weight", f"mesusie_pop{idx+1}", "enet_weight", "lasso_weight", "gblup_weight"]]
    ...
    tmp_res2 = pd.concat([tmp_fam[["iid"]], pd.DataFrame(imp_pheno)], axis=1)
    tmp_res2.columns = ["IID", "sushie", "indep", "meta", "mega", "mesusie","enet", "lasso", "ridge"]
    tmp_res2.to_csv(f"{args.out}.{cross_list[idx]}.cross.sscore", sep="\t", index=False)
