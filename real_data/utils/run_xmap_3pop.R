...
xmap_res <- tryCatch({
  fit_step1 <- XMAP::estimate_gc(
    ...
    df_ldscore$X1[keep_idx], df_ldscore$X2[keep_idx], df_ldscore$X4[keep_idx],
  ...
  fit_step2 <- XMAP::estimate_gc(
    ...
    df_ldscore$X1, df_ldscore$X2, df_ldscore$X4,
  ...
  OmegaHat <- diag(3)
  OmegaHat[1, 1] <- fit_step2$tau1$coefs[2]
  OmegaHat[2, 2] <- fit_step2$tau2$coefs[2]
  OmegaHat[1, 2] <- fit_step2$theta$coefs[2]
  ...
  fit_step1 <- XMAP::estimate_gc(
    data.frame(Z = df_ss1$Z[keep_idx], N = df_ss1$N[keep_idx]),
    data.frame(Z = df_ss3$Z[keep_idx], N = df_ss3$N[keep_idx]),
    df_ldscore$X1[keep_idx], df_ldscore$X3[keep_idx], df_ldscore$X5[keep_idx],
    reg_w1 = ld_w1[keep_idx], reg_w2 = ld_w3[keep_idx],
    reg_wx = sqrt(ld_w1[keep_idx] * ld_w3[keep_idx]),
    constrain_intercept = F)
  
  # stage 2 of LDSC: fix intercepts and estimate slopes
  fit_step2 <- XMAP::estimate_gc(
    data.frame(Z = df_ss1$Z, N = df_ss1$N),
    data.frame(Z = df_ss3$Z, N = df_ss3$N),
    df_ldscore$X1, df_ldscore$X3, df_ldscore$X5,
    reg_w1 = ld_w1, reg_w2 = ld_w3,
    reg_wx = sqrt(ld_w1 * ld_w3),
    constrain_intercept = T, fit_step1$tau1$coefs[1], fit_step1$tau2$coefs[1], fit_step1$theta$coefs[1])
  
  OmegaHat[3, 3] <- fit_step2$tau2$coefs[2]
  OmegaHat[1, 3] <- fit_step2$theta$coefs[2]
  
  c3 <- fit_step2$tau2$coefs[1] 
  
  fit_step1 <- XMAP::estimate_gc(
    data.frame(Z = df_ss2$Z[keep_idx], N = df_ss2$N[keep_idx]),
    data.frame(Z = df_ss3$Z[keep_idx], N = df_ss3$N[keep_idx]),
    df_ldscore$X2[keep_idx], df_ldscore$X3[keep_idx], df_ldscore$X6[keep_idx],
    reg_w1 = ld_w2[keep_idx], reg_w2 = ld_w3[keep_idx],
    reg_wx = sqrt(ld_w2[keep_idx] * ld_w3[keep_idx]),
    constrain_intercept = F)
  ...
  fit_step2 <- XMAP::estimate_gc(
    data.frame(Z = df_ss2$Z, N = df_ss2$N),
    data.frame(Z = df_ss3$Z, N = df_ss3$N),
    df_ldscore$X2, df_ldscore$X3, df_ldscore$X6,
    reg_w1 = ld_w2, reg_w2 = ld_w3,
    reg_wx = sqrt(ld_w2 * ld_w3),
    constrain_intercept = T, fit_step1$tau1$coefs[1], fit_step1$tau2$coefs[1], fit_step1$theta$coefs[1])
  
  OmegaHat[2, 3] <- fit_step2$theta$coefs[2]
  OmegaHat[lower.tri(OmegaHat)] <- OmegaHat[upper.tri(OmegaHat)]
  
  ar_sigma <- array(rep(c(1e-3, 1e-4, 1e-4,
    1e-4,1e-3,1e-4,
    1e-4,1e-4,1e-3), 10), dim = c(3, 3, 10))
  
  if (any(is.nan(OmegaHat))) {
    stop("OmegaHat contains nan")
  }
  
  xmap_res <- XMAP::XMAP(simplify2array(list(df_ld1, df_ld2, df_ld3)),
    cbind(df_ss1$Z, df_ss2$Z, df_ss3$Z),
    n = num_N, K = 10, Sigma = ar_sigma,
    Omega = OmegaHat, Sig_E = c(1, 1, 1),
    maxIter = 10, tol = 1e-4,
  ...
  print("xmap cannot run/converge")
  q()
})
...
if (nrow(df_cs) != 0) {
  ...
  for (idx in 1:10) {
    ...
    for (jdx in 1:3) {
      for (kdx in 1:3) {
        if (jdx < kdx) {
          tmp_rho <- tmp_matrix[jdx, kdx] /
            (sqrt(tmp_matrix[jdx, kdx]) * sqrt(tmp_matrix[jdx, kdx]))  
          
          idx_rho <- idx_rho %>%
            bind_rows(
              tibble(type = glue("ancestry{jdx}_ancestry{kdx}_est_corr"),
                est_corr = tmp_rho),
            )
        }
      }
    ...
    idx_rho$frob <- tmp_frob
    df_rho <- bind_rows(df_rho, idx_rho)
  }
