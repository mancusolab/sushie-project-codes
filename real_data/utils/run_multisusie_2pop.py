...
for idx in range(n_pop):
    ...
    pd.DataFrame(tmp_ld). \
        to_csv(f"{args.tmp_out}.inld.ans{idx}.tsv", index=False, sep="\t", header=None, float_format="%.12f")
...
#         df_tmp["pip"] = multisusie1.pip[multisusie1.sets[0][idx]]
...
#     df_cs = df_cs[["chrom", "snp", "pos", "a0", "a1", "CSIndex", "SNPIndex", "pip", "trait"]]
...
#     df_weights["pip_all"] = multisusie1.pip
#     df_weights["trait"] = args.trait
#     df_weights["method"] = "multisusie"
#     df_weights.to_csv(f"{args.out}/weights/multisusie.{args.trait}.weights.tsv", index=False, sep="\t")
#
