...
df_ld3 <- read_tsv(ld3, col_names = FALSE) %>%
  as.matrix()
new_matrix <- df_ld3
diag(new_matrix) <- 1
...
keep_idx <- which(df_ss1$Z^2 < 30 & df_ss2$Z^2 < 30 & df_ss3$Z^2 < 30)

ld_w1 <- 1 / sapply(df_ldscore$X1, function(x) max(x, 1))
ld_w2 <- 1 / sapply(df_ldscore$X2, function(x) max(x, 1))
ld_w3 <- 1 / sapply(df_ldscore$X3, function(x) max(x, 1))

total_df_ld <- num_N[1]/sum(num_N) * df_ld1 + num_N[2]/sum(num_N) * df_ld2 +
  num_N[3]/sum(num_N) * df_ld3

save(df_ss1, df_ss2, df_ss3, df_ld1, df_ld2, df_ld3, df_ldscore,
   ld_w1, ld_w2, ld_w3, ss_list, ld_list, df_ref,
