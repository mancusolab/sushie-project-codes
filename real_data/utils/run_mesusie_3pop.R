...
mesusie_res <- tryCatch({
  mesusie_res <- MESuSiE::meSuSie_core(ld_list,ss_list, L=10, estimate_residual_variance =TRUE, max_iter =500)
  mesusie_res
}, error = function(e) {
...
if (nrow(df_cs) != 0) {
  ...
  res <- tibble(as.data.frame(cbind(rowSums(res1), rowSums(res2), rowSums(res3))))
  df_ref$pop1_weights <- rowSums(res1)
  df_ref$pop2_weights <- rowSums(res2)
  df_ref$pop3_weights <- rowSums(res3)
