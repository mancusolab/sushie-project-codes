...
bigTMP=/scratch1/zeyunlu/temp_rnaseq/tempf_${NR}
...
do
 ...
 --output $TMPDIR/${ID}.normal
  ...
  wfile=$TMPDIR/${ID}.normal.sushie.weights.tsv
  ...
  do
    KEEP=${SCRATCH}/mesa_rnaseq_pt_${pop}.tsv
    COVAR=${SCRATCH}/mesa_rnaseq_covar_${pop}.tsv.gz

    # perform eQTLscan using plink
    ${PLINK} --bfile $TMPDIR/${ID}.${pop}.geno --extract  $TMPDIR/${ID}.snp --keep $KEEP --glm 'hide-covar' omit-ref \
    --covar 'iid-only' $COVAR --covar-variance-standardize --pheno 'iid-only' $TMPDIR/${ID}.${pop}.pheno \
    --out ${TMPDIR}/${ID}.${pop}
  done

  # run multisusie
  python ${SCRIPTF}/sushie-data-codes/real_data/utils/run_multisusie_3pop_test.py \
  ...
  sushie finemap --summary \
    --gwas $TMPDIR/${ID}.tmp.inss.ans0.tsv $TMPDIR/${ID}.tmp.inss.ans1.tsv $TMPDIR/${ID}.tmp.inss.ans2.tsv \
    --ld $TMPDIR/${ID}.tmp.inld.ans0.tsv $TMPDIR/${ID}.tmp.inld.ans1.tsv $TMPDIR/${ID}.tmp.inld.ans2.tsv \
    --sample-size $ss1 $ss2 $ss3 --alphas \
    --gwas-header chrom snp pos a1 a0 T_STAT \
    --out $TMPDIR/${ID}.sushie_ss
  ...
  Rscript ${SCRIPTF}/sushie-data-codes/real_data/utils/prepare_3pop.R \
    ...
    398:297:261 ${ID} $TMPDIR/prepare.${ID}.rdata
