...
p_ld3 <- plotHicTriangle(df_ld3, chrom=target_chrom, resolution = 25000,
  chromstart = target_tss, chromend = target_tes, zrange = c(0,max_counts),
  x=0.5 + 1.8*2 + 0.2*2, y=super_base , width = 1.8,
  height = base_height*1.8, just = c("left", "top"),
  palette = colorRampPalette(brewer.pal(n = 9, "BuPu")),
  default.units = "inches")
...
annoHeatmapLegend(
  plot = p_ld3,
  x = 0.5 + 1.8*3 + 0.2*2 + 0.1, y = super_base,
...
p4 <- plotManhattan(data = df_weight, chrom = target_chrom,
  ...
  leadSNP = list(snp = "", pch = 18, cex = 0.6, fill = "red"),
  fill="grey", assembly = "hg38", x = 0.5,
  y = 1.3+super_base + v_space*3 + base_height*3,
...
plotText(label = "PIP", x = 0.2,
  y = 1.3+super_base + v_space*3 + base_height*3 + (base_height*0.75) / 2,
...
p5 <- plotGenes(assembly = "hg38", chrom = target_chrom, chromstart = target_tss,
  ...
  y = 1.3+super_base + v_space*4 + base_height*3.75,
  width=figure_width, height = base_height)

plotText(label = "B", x = 0.1, y = 1.3+super_base + v_space*4 + base_height*4.75,
  fontsize = legend_font_size, fontface = "bold", just = "center",
  default.units = "inches")
...
p6 <- plotSignal(data = df_h3k27ac,
  ...
  x = 0.5, y =1.3+ super_base + v_space*4 + base_height*4.75 + 0.2,
...
plotText(label = "H3K27ac", x = 0.1,
  y = 1.3+super_base + v_space*4 + base_height*4.75 + 0.2 + (base_height*0.75) / 2,
...
p7 <- plotRanges(
  ...
  x = 0.5, y = 1.3+super_base + v_space*5 + base_height*5.5 + 0.2,
  width = figure_width, height = base_height*0.5,
  default.units = "inches"
)

plotText(label = "PBMC", x = 0.2,
  y = 1.3+super_base + v_space*5 + base_height*5.5 + 0.2 + (base_height*0.5)/2,
  rot = 90,
...
p8 <- plotRanges(
  ...
  linecolor = "fill",
  order = "random", collapse = TRUE,
  x = 0.5, y = 1.3+super_base + v_space*6 + base_height*6 + 0.2,
  width = figure_width, height = base_height*0.5,
  default.units = "inches"
)

plotText(label = "Naive T", x = 0.2,
  y = 1.3+super_base + v_space*6 + base_height*6 + 0.2 + (base_height*0.5)/2,
  rot = 90,
  fontsize = annot_font_size, fontface = "bold", default.units = "inches")
...
p9 <- plotRanges(
  ...
  linecolor = "fill",
  order = "random", collapse = TRUE,
  x = 0.5, y =1.3+ super_base + v_space*7 + base_height*6.5 + 0.2,
  width = figure_width, height = base_height*0.5,
  default.units = "inches"
)

plotText(label = "Naive B", x = 0.2,
  y =1.3+ super_base + v_space*7 + base_height*6.5 + 0.2 + (base_height*0.5)/2,
  rot = 90,
  fontsize = annot_font_size, fontface = "bold", default.units = "inches")
...
p10 <- plotRanges(
  ...
  linecolor = "fill",
  order = "random", collapse = TRUE,
  x = 0.5, y =1.3+ super_base + v_space*8 + base_height*7 + 0.2,
  width = figure_width, height = base_height*0.5,
  default.units = "inches"
)

plotText(label = "cNK", x = 0.2,
  y = 1.3+super_base + v_space*8 + base_height*7 + 0.2 + (base_height*0.5)/2,
  rot = 90,
  fontsize = annot_font_size, fontface = "bold", default.units = "inches")
...
p11 <- plotRanges(
  ...
  linecolor = "fill",
  order = "random", collapse = TRUE,
  x = 0.5, y = 1.3+super_base + v_space*9 + base_height*7.5 + 0.2,
  width = figure_width, height = base_height*0.5,
  default.units = "inches"
)

plotText(label = "Monocyte", x = 0.2,
  y = 1.3+super_base + v_space*9 + base_height*7.5 + 0.2 + (base_height*0.5)/2,
  rot = 90,
  fontsize = annot_font_size, fontface = "bold", default.units = "inches")

annoGenomeLabel(plot= p8, x = 0.5,
  y = 1.3+super_base + v_space*9 + base_height*8.25 + 0.2,
