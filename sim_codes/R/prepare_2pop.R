...
df_x1 <- as.matrix(read_tsv(x1, col_names = FALSE))
df_x2 <- as.matrix(read_tsv(x2, col_names = FALSE))
df_y1 <- as.matrix(read_tsv(y1, col_names = FALSE))
df_y2 <- as.matrix(read_tsv(y2, col_names = FALSE))

save(df_ss1, df_ss2, df_ld1, df_ld2, df_ldscore,
   ld_w1, ld_w2, ss_list, ld_list, df_ref,
  num_N, keep_idx, total_df_ld, trait_name, df_x1, df_x2, df_y1, df_y2,
  file = out)
