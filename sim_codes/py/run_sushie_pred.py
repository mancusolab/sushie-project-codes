...
def main(args):
    ...
    df_r2.to_csv(f"{args.output}.sim{args.sim}.locus{args.locus}.r2.tsv", sep="\t", index=False)
    ...
    z_1, _ = hf.compute_twas(gwas1, pop1_weight, output_dic["LD"][0])
    z_2, _ = hf.compute_twas(gwas2, pop2_weight, output_dic["LD"][1])

    df_z = pd.DataFrame(data=np.append(z_1[np.newaxis, :], z_2[np.newaxis, :], axis=0),
                        columns=["sushie", "indep", "meta", "susie",
                                 "enet", "lasso", "ridge"],
                        index=[1, 2]).reset_index(names="ancestry")

    df_z = add_annotation(df_z, args)

    df_z.to_csv(f"{args.output}.sim{args.sim}.locus{args.locus}.twas.tsv", sep="\t", index=False)
