...
gene_list <- read_tsv("../sim_data_prepare/sim_gene_index_list.tsv", col_names = FALSE) %>%
  select(gene = X3, LOCUS=X1, CHR=X2) %>%
...
tt <- bind_rows(tt1, tt2, tt3, tt4, tt5, tt6, tt7, tt8, tt9) %>%
  distinct() %>%
...
tt <- bind_rows(tt1, tt2, tt3, tt4, tt5) %>%
  distinct() %>%
  rownames_to_column("ROW") 
